...
ansible-playbook -i $host_file_name create_network.yml -e "host=$hypervisoruser@$hypervisorip name=$mgmtName xmlname=XMLs/$mgmtName-L2.xml count=$hypervisorcount mgmthypip=$mgmthypervisorip"
ip link add name $mgmtName-vx-$hypervisorcount type vxlan id $hypervisorcount dev $mgmtName-br remote $hypervisorip dstport 4789
...
ansible-playbook -i $host_file_name create_network.yml -e "host=$hypervisoruser@$hypervisorip name=$subnetname xmlname=XMLs/$subnetname.xml count=$hypervisorcount mgmthypip=$mgmthypervisorip"
ip link add name $subnetname-vx-$hypervisorcount type vxlan id $hypervisorcount dev $subnetname-br remote $hypervisorip dstport 4789
...
ansible-playbook -i inventory create_vm.yml -e "host=$hypervisoruser@$hypervisorip vmname=$vmname xmlname=$fullxmlpath imgpath=$imgpath"
...
jq '.ControllerIP = "'$controller_ip'"' > tmp.$$.json && mv tmp.$$.json mgmtdetails.json
jq '.ControllerNetworkName = "'$mgmtName'"' > tmp.$$.json && mv tmp.$$.json mgmtdetails.json
jq '.vethIP = "'$vethIP'"' > tmp.$$.json && mv tmp.$$.json mgmtdetails.json
